...
Stream.prototype._transform = function _transform( chunk, encoding, clbk ) {
	...
	// If we have split data, concat any previous partial split, re-encode (if need be) each split according to its original encoding, push each split to the destination, and set anything leftover as the new split buffer...
	else {
		split = this._buffer + splits[ 0 ];
		split = encode( split, encoding );
		this.push( split, encoding );
		for ( i = 1; i < len; i++ ) {
			split = encode( splits[ i ], encoding );
			this.push( split, encoding );
		}
		this._buffer = splits[ len ];
	}
...
Stream.prototype._flush = function _flush( clbk ) {
	...
	if ( split ) {
		split = encode( split, this._encoding );
